## restaurant pizza PostgreSQL Homework
...
con <- dbConnect(
  ...
  dbname = "qlongdyo",
  user = "qlongdyo",
  password = "QOOuKOUOBqUPrZrDY1SMEnGkJa6L2wum",
  port = 5432
)
...
customers_query <- dbGetQuery(con, "SELECT * FROM customers")

# Query to retrieve data from the 'orders' table
orders_query <- dbGetQuery(con, "SELECT * FROM orders")

# Query to retrieve data from the 'menu' table
menu_query <- dbGetQuery(con, "SELECT * FROM menu")
